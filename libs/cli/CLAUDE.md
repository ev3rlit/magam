...
HTTP server, transpiler, and executor for GraphWrite. Takes user `.tsx` files, transpiles them with esbuild, executes them to produce React elements, and renders to graph AST via the core reconciler.
...
- **`init [dir]`** — Creates `.graphwrite/` project scaffolding

On startup, calls `createCoreInterceptor()` from `@graphwrite/shared` to set up Module._load interception for monorepo development.

## HTTP Server (`src/server/http.ts`)

Port: `GRAPHWRITE_HTTP_PORT` env var or 3002 default. CORS enabled for all origins.
...
- **External**: `react`, `graphwrite`, `@graphwrite/core` — not bundled, resolved at runtime via require shim
...
2. `resolveModulePaths(cwd)` from `@graphwrite/shared`
...
The require shim is critical — it makes `require('@graphwrite/core')` and `require('react')` work inside temp files that aren't in the project's node_modules tree.

## WebSocket Server (`src/server/websocket.ts`)

Port: `GRAPHWRITE_WS_PORT` env var or 3001 default. Auto-discovery: tries up to 10 ports if preferred is taken.
...
Creates `.graphwrite/node_modules/graphwrite/index.d.ts` with mock type declarations for IDE support. Creates `tsconfig.json` with path alias (only if one doesn't exist — never overwrites).
