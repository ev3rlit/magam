...
5. variant 전체를 v1 필수로 확정하지 않는다. 현재는 제한된 `dark:`/`md:`만 우선 지원하고 `hover:` 같은 상호작용 variant는 후속 범위로 남긴다.
...
- variant 추가 확장 (`hover:` 등 상호작용 variant와 더 넓은 responsive subset)
...
2. `dark:`/`md:` 이후에 어떤 pseudo/responsive variant까지 확장할 것인가
