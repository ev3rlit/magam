...
kanban
    ...
    task_cli[CLI 프로그램<br/>명령줄에서 프로젝트 초기화·생성·빌드를 일관되게 처리<br/>Magam 워크플로를 스크립트 기반으로 자동화]@{ priority: 'High' }
  inProgress[진행중]
    ...
    task_obsidian[Obsidian 플러그인<br/>노트형 워크플로우 사용자도 자연스럽게 진입<br/>Magam 그래프를 기존 지식베이스와 연결]@{ priority: 'High' }
  ...
  planned[개발 예정]
    ...
    task_sticker[스티커 기능<br/>이미지에 스티커 아웃라인을 적용해 실제 스티커처럼 표현<br/>텍스트/이모지 스티커를 캔버스 오브젝트로 배치하고 anchor 고정 지원]@{ priority: 'High' }
    ...
    task_vscode[VS Code 확장<br/>Magam를 코드 편집기 내에서 직접 다루는 경로<br/>편집↔뷰 흐름을 끊김 없이 연결]@{ priority: 'Neutral' }
